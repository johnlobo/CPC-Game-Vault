    ...
    >
      <Card className="relative h-full flex flex-col overflow-hidden transition-all duration-300 ease-in-out group-hover:shadow-2xl group-hover:border-primary/50 hover:scale-[1.02] focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background rounded-lg">
        ...
        <CardContent className="flex-grow p-2 space-y-1">
          <CardTitle className="text-2xl line-clamp-2 group-hover:text-primary transition-colors">{game.title}</CardTitle>
          <div className="flex items-center text-xl text-muted-foreground space-x-1.5">
            <div className="flex items-center">
              <CalendarDays size={22} className="mr-0.5" />
              <span>{game.year}</span>
            </div>
            <div className="flex items-center">
              <Tag size={22} className="mr-0.5" />
              <span className="line-clamp-1">{game.genre}</span>
            </div>
          </div>
          <p className="text-xl text-muted-foreground line-clamp-2">{game.publisher}</p>
        </CardContent>
        <CardFooter className="p-2 pt-0.5">
          <div className="text-2xl text-accent flex items-center font-medium">
            Play <ArrowRight size={22} className="ml-1 transition-transform duration-300 ease-in-out group-hover:translate-x-0.5" />
          </div>
        </CardFooter>
      </Card>
    </Link>
